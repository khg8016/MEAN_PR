...
var getErrorMessage = function(err){ //err은 mongoose error 객체
    var message = '';
...
exports.renderSignin = function(req, res, next) { //로그인하기 창으로
    ...
    if (!req.user) {
        res.render('signin', {
            ...
            messages: req.flash('error') || req.flash('info') //error message를 flash 객체에 기록
        });
    } else { //로그인 됐으면
        return res.redirect('/');
    }
    ...
    if (!req.user) {//로그인 안됐으면
        ...
        user.save(function(err) {
            ...
            if (err) {
                ...
                req.flash('error', message);//error message를 flash 객체에 기록 view에서 띄워줌
